...
`POST /profile/news` и `PUT /profile/news/{id}` принимают необязательные `employee_ids` (до 20 уникальных ID), `gallery_urls` (до 20 изображений) и `attachments` (до 10 файлов по 10 МБ). Метаданные файла содержат `url`, `name`, `size` и `content_type`; все URL должны вести в хранилище платформы. Для новости организации разрешены только её сотрудники, для новости лаборатории — только сотрудники этой лаборатории. Публичные и управляющие ответы возвращают выбранных сотрудников, фотоальбом и файлы.

При редактировании владелец может передать полный набор `scope`, `organization_id`, `laboratory_id` и перенести публикацию в другой доступный ему источник. Противоречивый или недоступный источник отклоняется, сотрудники проверяются уже относительно нового владельца.
